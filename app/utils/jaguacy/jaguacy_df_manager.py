class JaguacyDataframeManager:
    ...
    def regroup_by_pallet_and_caliber(df):
        ...
        if "Cartons per pallet" not in grouped_df.columns:
            raise ValueError("❌ 'Quantity per grower' est requis pour calculer 'Nb of pallets'.")

        total = grouped_df.groupby("Pallet n°")["Cartons per pallet"].transform("sum")
        grouped_df["Nb of pallets"] = grouped_df["Cartons per pallet"] / total
