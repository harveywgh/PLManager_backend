class IngophaseDataframeManager:
    @staticmethod
    ...
    def regroup_by_pallet_and_caliber(df):
        ...
        # 🔹 Validation
        for col in ["Pallet_Id", "Count"]:
            if col not in df.columns:
                raise ValueError(f"❌ Colonne manquante : {col}")

        # 🔹 Agrégation
        grouped_df = df.groupby(["Pallet_Id", "Count"], as_index=False).agg(
            {col: "sum" if col in sum_cols else "first" for col in df.columns if col != "Nb of pallets"}
        )

        # 🔹 Calcul de Nb of pallets basé uniquement sur Cartons
        if "Cartons" not in grouped_df.columns:
            raise ValueError("❌ 'Cartons' est requis pour calculer 'Nb of pallets'.")

        total = grouped_df.groupby("Pallet_Id")["Cartons"].transform("sum")
