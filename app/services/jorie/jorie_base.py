from ...utils.jorie.jorie_loader import JorieLoader
from ...utils.jorie.jorie_df_manager import JorieDataframeManager
from ...utils.jorie.jorie_container_manager import JorieContainerManager
from ...utils.csv_manager import CSVManager
import os
import pandas as pd


class BaseJorieService:
    ...
    def _prepare_dataframe(self, file_path):
        dataframe = JorieLoader.load_excel_file(file_path)
        JorieDataframeManager.normalize_columns(dataframe)
        JorieDataframeManager.validate_columns(dataframe, self.pl_column_mapping)
        JorieDataframeManager.add_missing_columns(dataframe, self.pl_column_mapping)
        dataframe = JorieDataframeManager.regroup_by_pallet_and_caliber(dataframe)
        return dataframe

    def _group_containers(self, dataframe):
        return JorieContainerManager.group_by_container(dataframe)

    def _filter_container(self, dataframe, container):
        return JorieContainerManager.filter_by_container(dataframe, "Container n°", container)
    ...
    def _get_exporter_ref(self, container_df):
        ...
        """
        voyage_vals = container_df.get("Voyage", pd.Series()).dropna()
        if not voyage_vals.empty:
            return str(voyage_vals.iloc[0]).strip()

        # 2️⃣ Sinon utilise "Container n°"
        container_vals = container_df.get("Container n°", pd.Series()).dropna()
        if not container_vals.empty:
            return str(container_vals.iloc[0]).strip()

        # 3️⃣ Sinon "Unknown"
        print("⚠️ Aucun 'Voyage' ni 'Container n°' trouvé, on utilise 'Unknown'")
        return "Unknown"
